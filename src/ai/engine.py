...
3. **content_zh**：一段结构化的中文房源介绍。请严格按以下格式生成，使用 emoji 作为视觉标识，行与行之间用换行符（\n）分隔：
   ...
   格式说明：
   - 第1行：标题，与 title_zh 保持一致
   - 第2行：空行
   - 第3行：📍 开头，后跟完整地址（英文原文，不要翻译）
   - 第4行：💰 开头，后跟价格
   - 第5行：空行
   - 第6行：核心参数，用｜分隔，格式为：X卧Y卫｜面积 sqft｜年份年建。若某项缺失则省略
   - 第7行：房型与关键特征，用｜分隔，例如：半独立联排｜双车库｜全装修地下室
   - 第8行：空行
   - 第9行：✨ 房屋亮点：
   - 第10行：空行
   - 第11行起：每条亮点前面用 ✅，每行一条，共5-8条
   - 亮点列表后：空行 + 2-3句简短的总结性描述
...
4. **highlights**：5-8个核心亮点短语（中文），每个短语简短有力，不含 emoji。例如："带双车库"、"全装修地下室"、"无遮挡绿地景观"
...
{{
    ...
    "content_zh": "结构化的中文房源介绍（含 emoji 分块标识）...",
...
3. **content_zh**：一段结构化的中文物业介绍。请严格按以下格式生成，使用 emoji 作为视觉标识，行与行之间用换行符（\n）分隔：
   ...
   - 第5行：🏢 开头，后跟物业类型与用途
   - 第6行：空行
   - 第7行：核心参数，用｜分隔，格式为：建筑面积 sqft｜占地面积 sqft｜年份年建。若某项缺失则省略
   - 第8行：关键特征，用｜分隔
   - 第9行：空行
   - 第10行：✨ 物业亮点：
   - 第11行：空行
   - 第12行起：每条亮点前面用 ✅，每行一条，共5-8条
   - 亮点列表后：空行 + 2-3句简短的客观总结
...
4. **highlights**：5-8个核心特点短语（中文），每个短语简短有力，只陈述事实，不含 emoji。例如"24间客房汽车旅馆"、"1980年建造"、"屋顶与窗户已更新"、"附带业主生活区"等
5. **year_built**：优先从 `raw_data.summary.built_in` 提取建造年份，如果不存在则从描述中提取，仍未找到则留空字符串
6. 地址、人名、机构名、MLS号码保持英文原文，不要翻译
7. 面积单位保持原文（sqft），可在括号内标注近似平方米
